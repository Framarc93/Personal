...
def eaMuPlusLambdaTol(population, toolbox, mu, lambda_, ngen, tol, stats=None, halloffame=None, verbose=__debug__):
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    gen = 1
    while gen < ngen + 1 and not (min_fit[-1][0] <= tol):
        # Vary the population
        offspring = varOr(population, toolbox, lambda_, cxpb, mutpb)
...
#animated_plot4 = \
plt.plot(tplot_new, Tplot_new, marker='.', color = 'k', label="ON DESIGN")
...
if __name__ == "__main__":
    ...
    def extrapolate():
        ...
        if count < len(hof):
            ...
            res = tools.initIterate(creator.Individual, toolbox.individual)
        return res
...
passint_c_plot = (tfin - change_time) * 2
tevals_c_plot = np.linspace(change_time, tfin, int(passint_c_plot))

if change_time + t_offdesign <= total_time_simulation:
    t_end = change_time + t_offdesign
else:
    t_end = total_time_simulation

solgp_c = solve_ivp(sys2GP_c, [change_time, t_end], xnew_ini, t_eval=tevals_c)
Tplot_c = []
tplot_c = []
solgp_c_plot = solve_ivp(sys2GP_c, [change_time, tfin], xnew_ini, t_eval=tevals_c_plot)
i=1
ind = np.zeros((0), dtype=int)
...
vout_c = solgp_c.y[1, :]
mout_c = solgp_c.y[2, :]
rout_c_p = solgp_c_plot.y[0, :]
vout_c_p = solgp_c_plot.y[1, :]
mout_c_p = solgp_c_plot.y[2, :]


for i in range(len(mout_c)):
    if mout_c[i] < obj.M0*obj.Mc:
        mout_c[i] = obj.M0*obj.Mc
for i in range(len(mout_c_p)):
    if mout_c_p[i] < obj.M0*obj.Mc:
        mout_c_p[i] = obj.M0*obj.Mc
ttgp_c = solgp_c.t
ttgp_c_p = solgp_c_plot.t
...
plt.figure(1)
#animated_plot_c = \
plt.plot(ttgp_c_p, (rout_c_p - obj.Re) / 1e3, marker='.', color = 'b', label="OFF DESIGN")
plt.figure(2)
#animated_plot_c2 = \
plt.plot(ttgp_c_p, vout_c_p, marker='.', color = 'b', label="OFF DESIGN")
plt.figure(3)
#animated_plot_c3 = \
plt.plot(ttgp_c_p, mout_c_p, marker='.', color = 'b',  label="OFF DESIGN")
plt.figure(4)
#animated_plot_c4 = \
plt.plot(tplotc_new, Tplotc_new, marker='.', color = 'b',  label="OFF DESIGN")
...
if t_end < total_time_simulation:
    solgp_gp = solve_ivp(sys2GP_gp, [t_end, total_time_simulation], xnew_ini_gp, t_eval=tevals_gp)
    ...
    plt.figure(2)
    #animated_plot_gp2 = \
    plt.plot(ttgp_gp, vout_gp, marker='.', color='g',  label="ONLINE CONTROL")
    plt.figure(3)
    #animated_plot_gp3 = \
    plt.plot(ttgp_gp, mout_gp, marker='.', color='g',  label="ONLINE CONTROL")
    plt.figure(4)
    #animated_plot_gp4 = \
    plt.plot(tplotgp_new, Tplotgp_new, marker='.', color='g',  label="ONLINE CONTROL")
