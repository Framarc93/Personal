...
def equality(prob, obj):
    ...
    # event condition
    result.equal(v[0], 1e-11, unit=prob.unit_states[0][0])
    ...
    result.equal(h[0], 1e-11, unit=prob.unit_states[0][5])
...
def inequality(prob, obj):
    ...
    # lower bounds
    result.lower_bound(v, 1e-11, unit=prob.unit_states[0][0])
    ...
    result.lower_bound(h, 1e-11, unit=prob.unit_states[0][5])
    ...
    result.lower_bound(delta, 1e-11, unit=prob.unit_controls[0][1])
...
def cost(prob, obj):
    ...
    return -mf[-1] / prob.unit_states[0][6]
...
cl = fileRead("clfile.txt")
cd = fileRead("cdfile.txt")
cm = fileRead("cmfile.txt")
...
flag_savefig = True
savefig_file = "Test_Plot_Goddardm15150_"
...
# velocity
v_init = Guess.cubic(prob.time_all_section, 1e-9, 0.0, obj.Vtarget, 0.0)
...
h_init = Guess.cubic(prob.time_all_section, 1e-9, 0.0, obj.Hini, 0.0)
m_init = Guess.cubic(prob.time_all_section, obj.M0, 0.0, obj.m10, 0.0)

alfa_init = Guess.zeros(prob.time_all_section)
delta_init = Guess.linear(prob.time_all_section, 1.0, 1e-9)
...
def display_func():
    ...
    #prob.set_states_all_section(1, savgol_filter(chi, 15, 3))
    #prob.set_states_all_section(2, savgol_filter(gamma, 15, 3))
    #prob.set_controls_all_section(0, savgol_filter(alfa, 15, 4))
    #prob.set_controls_all_section(1, savgol_filter(delta, 15, 2))
    #prob.set_controls_all_section(2, savgol_filter(deltaf, 15, 4))  # window size 19, polynomial order 3
    #prob.set_controls_all_section(3, savgol_filter(tau, 15, 2))
    #prob.set_controls_all_section(4, savgol_filter(mu, 15, 4))



prob.solve(obj, display_func, ftol=1e-12, eps=1e-12, maxiter=30)
...
T, Deps, isp, MomT = obj.thrust(Press, m, presv, spimpv, delta, tau, n[0], obj.psl, obj.M0, obj.m10, obj.lRef, obj.xcgf, obj.xcg0)
T = np.asarray(T, dtype=np.float64)
Deps = np.asarray(Deps, dtype=np.float64)
MomT = np.asarray(MomT, dtype=np.float64)
...
plt.plot(time, np.rad2deg(mu), marker="o", label="Mu")
for line in prob.time_knots():
    plt.axvline(line, color="k", alpha=0.5)
plt.grid()
plt.xlabel("time [s]")
