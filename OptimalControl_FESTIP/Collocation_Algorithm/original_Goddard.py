...
from OpenGoddard.optimize import Guess, Problem, Condition, Dynamics
...
def SingleShooting(states, controls, dyn, time, Nint):
    ...
    Tr_Int = interpolate.PchipInterpolator(time, controls[0, :])
    Tt_Int = interpolate.PchipInterpolator(time, controls[1, :])
    ...
    for i in range(Nint - 1):
        # print(i, x[i,:])
        # print(u[i,:])
        k1 = dt * dyn(t[i], x[i, :], Tr_Int, Tt_Int)
        # print("k1: ", k1)
        k2 = dt * dyn(t[i] + dt / 2, x[i, :] + k1 / 2, Tr_Int, Tt_Int)
        # print("k2: ", k2)
        k3 = dt * dyn(t[i] + dt / 2, x[i, :] + k2 / 2, Tr_Int, Tt_Int)
        # print("k3: ", k3)
        k4 = dt * dyn(t[i + 1], x[i, :] + k3, Tr_Int, Tt_Int)
        # print("k4: ", k4)
    ...
    Vtres = x[:, 3]
    mres = x[:, 4]
    Trres = Tr_Int(time_new)
    Ttres = Tt_Int(time_new)

    return Rres, tetares, Vrres, Vtres, mres, Trres, Ttres, time_new
...
Uval = np.vstack((Tr, Tt))

Xval = np.array((R, theta, Vr, Vt, m))
...
plt.figure()
plt.title("theta")
plt.plot(time, np.rad2deg(theta), "o", label="Theta")
...
plt.figure()
plt.title("Velocity")
plt.plot(time, Vr, "o", label="Vr")
plt.plot(time, Vt, "o", label="Vt")
plt.plot(tres, Vrres, label="Vr Integration")
plt.plot(tres, Vtres, label="Vt Integration")
for line in prob.time_knots():
    plt.axvline(line, color="k", alpha=0.5)
plt.grid()
plt.xlabel("time [s]")
