...
class Spaceplane():
    ...
    def __init__(self):
        ...
        self.alfa_lb = np.hstack(
            (np.repeat(to_new_int(np.deg2rad(-1), np.deg2rad(-2), np.deg2rad(40), 0.0, 1.0), self.n15),
             np.repeat(to_new_int(np.deg2rad(-2), np.deg2rad(-2), np.deg2rad(40), 0.0, 1.0), self.n85)))

        self.alfa_ub = np.hstack(
            (np.repeat(to_new_int(np.deg2rad(1), np.deg2rad(-2), np.deg2rad(40), 0.0, 1.0), self.n15),
        ...
        self.mu_ub = np.hstack(
            (np.repeat(to_new_int(np.deg2rad(1.0), np.deg2rad(-80), np.deg2rad(80), 0.0, 1.0), self.n15),
             np.repeat(to_new_int(np.deg2rad(60), np.deg2rad(-60), np.deg2rad(80), 0.0, 1.0),
                       n[0] - 2 * self.n15),
             np.repeat(to_new_int(np.deg2rad(1.0), np.deg2rad(-80), np.deg2rad(80), 0.0, 1.0), self.n15)))

        self.gamma_lb = np.hstack(
            (np.repeat(to_new_int(self.gammastart, np.deg2rad(-89.9), np.deg2rad(89.9), 0.0, 1.0), self.n20),
             np.repeat(to_new_int(np.deg2rad(-89.9), np.deg2rad(-89.9), np.deg2rad(89.9), 0.0, 1.0), self.n80)))

        self.chi_ub = np.hstack(
            (np.repeat(to_new_int(np.deg2rad(130), np.deg2rad(110), np.deg2rad(150), 0.0, 1.0), self.n10),
...
def dynamics(prob, obj, section):
    vx = prob.states(0, section)
    vy = prob.states(1, section)
    vz = prob.states(2, section)
    wx = prob.states(3, section)
    wy = prob.states(4, section)
    wz = prob.states(5, section)
    C1 = prob.states(6, section)
    C2 = prob.states(7, section)
    C3 = prob.states(8, section)
    C4 = prob.states(9, section)
    C5 = prob.states(10, section)
    C6 = prob.states(11, section)
    C7 = prob.states(12, section)
    C8 = prob.states(13, section)
    C9 = prob.states(14, section)
    x = prob.states(15, section)
    y = prob.states(16, section)
    z = prob.states(17, section)
    ...
    Tx, Tz, Deps, isp, MomT = obj.thrust(Press, m, presv, spimpv, delta, tau, prob.nodes[0], obj.psl, obj.M0, obj.m10,
    ...
    dx[0] = vx*wz-vz*wy + Tx/m + (L*np.sin(alfa)-D*np.cos(alfa))/m
    dx[1] = vz*wx-vx*wz
    dx[2] = vx*wy - vy*wx + Tz/m + m*g + + (L*np.cos(alfa)-D*np.sin(alfa))/m
    dx[3] = wz*wy*(obj.I[2,2]-obj.I[3,3])/obj.I[1,1]
    dx[4] = wz*wx*(obj.I[3,3]-obj.I[1,1])/obj.I[2,2] + (MomTot)/obj.I[2,2]
    dx[5] = wy*wx*(obj.I[1,1]-obj.I[2,2])/obj.I[3,3]
    dx[6] = -T / (g0 * isp)
    dx[7] = wz*C4 - wy*C7
    dx[8] = wz * C5 - wy * C8
    dx[9] = wz * C6 - wy * C9
    ...
    dx[14] = wy * C2 - wx * C5
    dx[15] = wy * C3 - wx * C6
    dx[16] = vx*C1 + vy*C4 + vz*C7
    dx[17] = vx*C2 + vy*C5 + vz*C8
    dx[18] = vx*C3 + vy*C6 + vz*C9
...
def inequality(prob, obj):
    ...
    result.upper_bound(mgamma, np.repeat(np.tan(1.4), n[0] - 1), unit=1)  # bound on slope of first gamma points
    result.upper_bound(malfa, np.repeat(np.tan(1.4), n[0] - 1), unit=1)  # bound on alfa slope
    # result.upper_bound(malfa[obj.n60:], np.repeat(np.tan(0.31), obj.n40 - 1), unit=1) # bound on slope of last alfa points
    result.upper_bound(mdeltaf, np.repeat(np.tan(1.4), n[0] - 1), unit=1)  # bound on deltaf slope
    # result.upper_bound(mdeltaf[obj.n20:obj.n80], np.repeat(np.tan(0.79), obj.n80-obj.n20), unit=1)  # bound on deltaf slo
    # result.upper_bound(mdeltaf[:obj.n20], np.repeat(np.tan(0.35), obj.n20), unit=1) # bound on last deltaf slope
    result.upper_bound(mthrot, np.repeat(np.tan(1.4), (n[0] - 1) * 2), unit=1)  # bound on thr ottles slope
    result.upper_bound(mmu, np.repeat(np.tan(1.4), n[0] - 1), unit=1)  # bound on mu slope
    # result.upper_bound(mmu[obj.n35:obj.n65], np.repeat(np.tan(0.79), obj.n65 - obj.n35), unit=1)
...
if __name__ == '__main__':
    ...
    unit_v = 1e4
    unit_w = np.deg2rad(150)
    unit_C = np.deg2rad(180)
    unit_pos = 2e5
    unit_m = obj.M0
    unit_t = 700
    ...
    prob.set_unit_states_all_section(3, unit_w)
    prob.set_unit_states_all_section(4, unit_w)
    prob.set_unit_states_all_section(5, unit_w)
    prob.set_unit_states_all_section(6, unit_m)
    prob.set_unit_states_all_section(7, unit_C)
    prob.set_unit_states_all_section(8, unit_C)
    prob.set_unit_states_all_section(9, unit_C)
    prob.set_unit_states_all_section(10, unit_C)
    prob.set_unit_states_all_section(11, unit_C)
    prob.set_unit_states_all_section(12, unit_C)
    prob.set_unit_states_all_section(13, unit_C)
    prob.set_unit_states_all_section(14, unit_C)
    prob.set_unit_states_all_section(15, unit_C)
    prob.set_unit_states_all_section(16, unit_pos)
    prob.set_unit_states_all_section(17, unit_pos)
    prob.set_unit_states_all_section(18, unit_pos)
    ...
    plt.figure(0)
    plt.title("Altitude profile Collocation Final")
