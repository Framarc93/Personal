...
Nint = 101 # number of points for each single shooting integration
maxiter = 100  # max number of iterations for nlp solver
ftol = 1e-8  # numeric tolerance of nlp solver
eps = 1e-10  # increment of the derivative?
...
X0 = np.hstack((X, U, dt / time_tot))  # vector of initial conditions here all the angles are in degrees!!!!!
...
bndX = ((1e-9 / unit_v, 1.0), (np.deg2rad(90) / unit_chi, np.deg2rad(270) / unit_chi),
        (np.deg2rad(0.0), np.deg2rad(90.0) / unit_gamma), (None, None),
...
def inequalityAll(states, controls):
    ...
    L, D, MomA = aeroForcesMulti(M, alfa, deltaf, cd, cl, cm, v, obj.wingSurf, rho, obj.lRef, obj.M0, m, obj.m10,
                                 obj.xcg0, obj.xcgf, obj.pref, NineqCond)

    L = np.asarray(L, dtype=np.float64)
    D = np.asarray(D, dtype=np.float64)
    MomA = np.asarray(MomA, dtype=np.float64)

    T, Deps, isp, MomT = thrustMulti(Press, m, presv, spimpv, delta, tau, NineqCond, obj.psl, obj.M0, obj.m10, obj.lRef,
                                     obj.xcgf, obj.xcg0)
    ...
    return np.hstack(((obj.MaxAx - ax)/obj.MaxAx, (obj.MaxAz - az)/obj.MaxAz, (obj.MaxQ - q)/obj.MaxQ,
                      (obj.k - MomTotA)/obj.k))  # when with bounds
...
def SingleShooting(states, controls, dyn, tstart, tfin, Nint):
    ...
    #x = np.zeros((Nstates))
    x = states[0:Nstates] * states_unit  # vector of intial states ready
    #print("Single: ", Time)
    ...
    #odeint(dyn, x, time_new, args=(alfa_Int, delta_Int, deltaf_Int, tau_Int))
    #sol = solve_ivp(dyn, [tstart, tfin], VAR, t_eval=t)
    #print(sol)
    ...
    return vres, chires, gammares, tetares, lamres, hres, mres, sol.t, alfares, deltares, deltafres, taures, \
...
def MultiShooting(var, dyn):
    ...
    for i in range(Nleg):
        ...
        if i == 0.0:
            timestart = 0.0
        timeend = timestart + var[varTot+i] * time_tot
...
def MultiShootIneq(var, dyn):
    ...
    for i in range(Nleg):
        ...
        ineq_cond = inequalityAll(states_after, controls_after)
        timestart = timeend
...
def MultiPlot(var, dyn):
    ...
    for i in range(Nleg):
        ...
        L, D, MomA = aeroForcesMulti(M, alfares, deltafres, cd, cl, cm, vres, obj.wingSurf, rho, obj.lRef, obj.M0, mres,
                                     obj.m10, obj.xcg0, obj.xcgf, obj.pref, Nint)
        L = np.asarray(L, dtype=np.float64)
        D = np.asarray(D, dtype=np.float64)
        MomA = np.asarray(MomA, dtype=np.float64)

        T, Deps, isp, MomT = thrustMulti(Press, mres, presv, spimpv, deltares, taures, Nint, obj.psl, obj.M0, obj.m10,
                                         obj.lRef, obj.xcgf, obj.xcg0)
        T = np.asarray(T, dtype=np.float64)
        Deps = np.asarray(Deps, dtype=np.float64)
        MomT = np.asarray(MomT, dtype=np.float64)
        ...
        ax = (T - D * np.cos(alfares) + L * np.sin(alfares)) / mres
        az = (T + D * np.sin(alfares) + L * np.cos(alfares)) / mres
        ...
        plt.plot(tres, np.rad2deg(chires), color="g", label="Chi")
        plt.plot(tres, np.rad2deg(gammares), color="b", label="Gamma")
        plt.plot(tres, np.rad2deg(tetares), color="r", label="Theta")
        plt.plot(tres, np.rad2deg(lamres), color="k", label="Lambda")
        plt.ylabel("deg")
        plt.xlabel("time [s]")
        plt.legend(loc="best")
        plt.axvline(tres[-1], color="k", alpha=0.5)
        ...
        plt.plot(tC, np.rad2deg(alfaCP[i, :]), 'ro', label="Control points")
        plt.ylabel("deg")
        plt.xlabel("time [s]")
        plt.legend(loc="best")
        plt.axvline(tres[-1], color="k", alpha=0.5)
        ...
        plt.figure(10)
        plt.title("Body Flap deflection")
        plt.plot(tres, np.rad2deg(deltafres))
        plt.plot(tC, np.rad2deg(deltafCP[i, :]), 'ro', label="Control points")
        plt.ylabel("deg")
        plt.xlabel("time [s]")
        plt.legend(loc="best")
        plt.axvline(tres[-1], color="k", alpha=0.5)
...
def equality(var):
    ...
    v = var[varStates - 7] * unit_v
    h = var[varStates - 2] * unit_h
    lam = var[varStates - 3] * unit_lam
    ...
    vtAbs, chiass, vtAbs2 = vass(var[varStates-7:varStates]*states_unit, var[varTot-4:varTot], dynamicsVel)
    ...
    eq_cond = np.concatenate((eq_cond, abs(var[Nstates:varStates] - conj[:Nstates*(Nbar-2)]))) # knotting conditions
