...
eps = 1e-12  # increment of the derivative?
...
Xub = ([np.inf, np.deg2rad(270)/unit_chi, np.deg2rad(89.9)/unit_gamma, np.inf, obj.incl/unit_lam, obj.Hini/unit_h, obj.M0/unit_m]) # states upper bounds

Ulb = ([np.deg2rad(-2)/unit_alfa, 0.001, np.deg2rad(-20)/unit_deltaf, 0.0]) # controls lower bounds
...
def inequalityAll(states, controls, rep):
    ...
    return np.hstack(((obj.MaxAx - ax)/(obj.MaxAx*100), (obj.MaxAz - az)/(obj.MaxAz*100), (obj.MaxQ - q)/(obj.MaxQ*1000),
                      (obj.k - MomTotA)/(obj.k*1000000), ((h[-1]-80000)/unit_h,)))  # when with bounds
...
def equality(var):
    ...
    if np.cos(obj.incl)/np.cos(lam) > 1 or np.cos(obj.incl)/np.cos(lam) < -1:
        chifin = np.pi
    else:
