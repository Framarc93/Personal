...
cl = fileRead("clMFormat.txt")
cd = fileRead("cdMFormat.txt")
...
Nint = 450 # number of points for each single shooting integration
maxiter = 20  # max number of iterations for nlp solver
ftol = 1e-12  # numeric tolerance of nlp solver
eps = 1e-13  # increment of the derivative?
...
Xub = ([10000/unit_v, np.deg2rad(270)/unit_chi, np.deg2rad(89.9)/unit_gamma, np.inf, obj.incl/unit_lam, obj.Hini/unit_h, obj.M0/unit_m]) # states upper bounds
...
def inequalityAll(states, controls, rep):
    ...
    return np.hstack(((obj.MaxAx - ax)/(obj.MaxAx*100), (obj.MaxAz - az)/(obj.MaxAz*100), (obj.MaxQ - q)/(obj.MaxQ*1000),
                      (obj.k - MomTotA)/(obj.k*1000000)))  # when with bounds
