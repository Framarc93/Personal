...
def inequalityAll(states, controls, varnum, time):
    ...
    plt.figure()
    plt.plot(time, (obj.MaxAx - ax)/obj.MaxAx, label="ax")
    plt.plot(time, (obj.MaxAz - az)/obj.MaxAz, label="az")
    plt.plot(time, (obj.MaxQ - q)/(obj.MaxQ), label="q")
    plt.plot(time, (obj.k - MomTotA)/(obj.k*1e3), label="mom")
    plt.legend(loc="best")
    plt.grid()
    plt.show()
    plt.close()
    ...
    iC = np.hstack((((obj.MaxAx - ax)/obj.MaxAx, (obj.MaxAz - az)/obj.MaxAz, (obj.MaxQ - q)/obj.MaxQ,
                      (obj.k - MomTotA)/(obj.k*1e3), (mf - obj.m10)/unit_m,)))  # when with bounds
...
def MultiShooting(var, dyn):
    ...
    for i in range(Nleg):
        ...
        totTime = np.hstack((totTime, tres))
        timestart = timeend
    ...
    ineq_Cond = inequalityAll(states_after, controls_after, Nint*Nleg, totTime)
    ...
    return eq_Cond, ineq_Cond, objective
...
def equality(var, conj):
    ...
    eq_cond = np.concatenate((eq_cond, (abs(var[varStates + 4]),)))  # init cond on mu
...
#save_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/sparse_festip_new11Mu.npz", sparse)