...
def SingleShooting(states, controls, dyn, tstart, tfin, Nint):
    ...
    chires = x[:, 1]
    gammares = x[:, 2]
    tetares = x[:, 3] + np.deg2rad(90)  # teta back to [0, 90]
    lamres = (x[:, 4] + obj.incl) / 2  # lambda back to [0, incl]
    hres = x[:, 5]
    mres = x[:, 6]
...
#save_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/sparse_festip_4leg9cont.npz", sparse)