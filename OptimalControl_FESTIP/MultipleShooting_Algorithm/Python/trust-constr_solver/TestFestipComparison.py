...
ftol = 1e-10  # numeric tolerance of nlp solver
...
Xlb = ([1e-5/unit_v, np.deg2rad(90), 0.0, -20000, -obj.incl/unit_lam, 1e-5/unit_h, obj.m10/unit_m]) # states lower bounds
Xub = ([1.0, np.deg2rad(270), np.deg2rad(89.9), 20000, obj.incl/unit_lam, obj.Hini/unit_h, obj.M0/unit_m]) # states upper bounds

Ulb = ([np.deg2rad(-2)/unit_alfa, 0.001, np.deg2rad(-20), 0.0]) # controls lower bounds
Uub = ([np.deg2rad(40)/unit_alfa, 1.0, np.deg2rad(30), 1.0]) # controls upper bounds
...
def inequalityAll(states, controls, varnum):
    ...
    iC = np.concatenate((((obj.MaxAx - ax)/(obj.MaxAx*1000), (obj.MaxAz - az)/(obj.MaxAz*1000), (obj.MaxQ - q)/(obj.MaxQ*1000),
                      (obj.k - MomTotA)/(obj.k*10000000))))  # when with bounds
...
def constraints(var):
    ...
    if all(j==0 for j in abs((var - obj.varOld))):
        return np.concatenate((obj.eqOld, obj.ineqOld))

    else:
        eq_c, ineq_c, fun = MultiShootingOne(var, dynamicsInt)
...
def cost_fun(var):
    ...
    if all(j==0 for j in abs((var - obj.varOld))):
        ...
        #print("costnew")
        eqcons, ineqcons, cost = MultiShootingOne(var, dynamicsInt)

        return cost

sparseJac = load_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/sparse_festip100.npz")
...
lb = lbeq * (Nstates+7+Nstates*(Nleg-1)) + lbineq * 4 * NineqCond * Nleg # all lower bounds
ub = ubeq * (Nstates+7+Nstates*(Nleg-1)) + ubineq * 4 * NineqCond * Nleg# all upper bounds
...
#save_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/sparse_festip100.npz", sparse)