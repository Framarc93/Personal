...
NContPoints = 15  # number of control points for interpolation inside each interval
...
NineqCond = int(Nleg * NContPoints - Nbar + 2)
...
chi_init = Guess.cubic(tnew, chi_toNew(obj.chistart), 0.0, chi_toNew(obj.chi_fin), 0.0)
...
delta_init = Guess.cubic(tcontr, 1.0, 0.0, 0.01, 0.0)
...
#Vlb = Xlb*Nleg + Ulb * (Nleg * NContPoints - Nbar + 2) + Tlb*Nleg
#Vub = Xub*Nleg + Uub * (Nleg * NContPoints - Nbar + 2) + Tub*Nleg
...
Vlb = Xlb*Nleg + Ulb * Ncontrols * (Nleg * NContPoints - Nbar + 2) + Tlb*Nleg
Vub = Xub*Nleg + Uub * Ncontrols * (Nleg * NContPoints - Nbar + 2) + Tub*Nleg
bnds = Bounds(Vlb, Vub)
...
def inequalityAll(states, controls, varnum):
    ...
    momnew = to_new_int(obj.k/ 1e5, 0.0, 1e3, 0.0, 1.0) - to_new_int(MomTotA / 1e5, 0.0, 1e3, 0.0, 1.0)
    mnew = m_toNew(mf, obj) - m_toNew(obj.m10, obj)

    iC = np.hstack((axnew, aznew, qnew, momnew, mnew))
...
def MultiShooting(var, dyn):
    ...
    vineql = np.zeros((1, NineqCond))  # states defined as row vectors
    chiineql = np.zeros((1, NineqCond))
    gammaineql = np.zeros((1, NineqCond))
    tetaineql = np.zeros((1, NineqCond))
    lamineql = np.zeros((1, NineqCond))
    hineql = np.zeros((1, NineqCond))
    mineql = np.zeros((1, NineqCond))
    ...
    alfaineql = np.zeros((1, NineqCond))
    deltaineql = np.zeros((1, NineqCond))
    deltafineql = np.zeros((1, NineqCond))
    tauineql = np.zeros((1, NineqCond))
    muineql = np.zeros((1, NineqCond))
    ...
    for i in range(Nleg):
        ...
        vres, chires, gammares, tetares, lamres, hres, mres, tres, alfares, deltares, deltafres, taures, mures, alfa_I, \
        delta_I, deltaf_I, tau_I, mu_I = SingleShooting(states, controls, dyn, timestart, timeend, Nint)
        ...
        vineq[0, :] = vres
        chiineq[0, :] = chires
        gammaineq[0, :] = gammares
        tetaineq[0, :] = tetares
        lamineq[0, :] = lamres
        hineq[0, :] = hres
        mineq[0, :] = mres
        alfaineq[0, :] = alfares
        deltaineq[0, :] = deltares
        deltafineq[0, :] = deltafres
        tauineq[0, :] = taures
        muineq[0, :] = mures
        ...
        if NineqCond != Nint:
            ...
            vineql[0, :] = vinterp(time_ineq)
            chiineql[0, :] = chiinterp(time_ineq)
            gammaineql[0, :] = gammainterp(time_ineq)
            tetaineql[0, :] = tetainterp(time_ineq)
            lamineql[0, :] = laminterp(time_ineq)
            hineql[0, :] = hinterp(time_ineq)
            mineql[0, :] = minterp(time_ineq)
            alfaineql[0, :] = alfa_I(time_ineq)
            deltaineql[0, :] = delta_I(time_ineq)
            deltafineql[0, :] = deltaf_I(time_ineq)
            tauineql[0, :] = tau_I(time_ineq)
            muineql[0, :] = mu_I(time_ineq)

            '''these are column vectors'''
            vtl = np.transpose(vineql)
            chitl = np.transpose(chiineql)
            gammatl = np.transpose(gammaineql)
            tetatl = np.transpose(tetaineql)
            lamtl = np.transpose(lamineql)
            htl = np.transpose(hineql)
            mtl = np.transpose(mineql)
            alfatl = np.transpose(alfaineql)
            deltatl = np.transpose(deltaineql)
            deltaftl = np.transpose(deltafineql)
            tautl = np.transpose(tauineql)
            muttl = np.transpose(muineql)

            if i == 0:
                states_afterl = np.column_stack((vtl, chitl, gammatl, tetatl, lamtl, htl, mtl))
                controls_afterl = np.column_stack((alfatl, deltatl, deltaftl, tautl, muttl))
            else:
                states_afterl = np.vstack((states_afterl, np.column_stack((vtl, chitl, gammatl, tetatl, lamtl, htl, mtl))))
                controls_afterl = np.vstack((controls_afterl, np.column_stack((alfatl, deltatl, deltaftl, tautl, muttl))))

        timestart = timeend

    h = h_toOrig(states_atNode[varStates - 2])
    m = m_toOrig(states_atNode[-1], obj)
    ...
    if NineqCond != Nint:
        ineq_Cond = inequalityAll(states_afterl, controls_afterl, len(states_afterl))  # evaluation of inequality constraints
    else:
        ineq_Cond = inequalityAll(states_after, controls_after,len(states_afterl))  # evaluation of inequality constraints
...
def plot(var):
    ...
    for i in range(Nleg):
        ...
        plt.plot(tC, np.rad2deg(alfaCP[i, :]), marker="o")
        ...
        plt.plot(tC, deltaCP[i, :] * 100, marker="o")
        plt.plot(tC, tauCP[i, :] * 100, marker="o")
        plt.ylabel("%")
        plt.xlabel("time [s]")
        ...
        plt.plot(tC, np.rad2deg(deltafCP[i, :]), marker="o")
        ...
        plt.plot(tC, np.rad2deg(muCP[i, :]), marker="o")
...
def equality(var, conj):
    ...
    eq_cond = np.zeros((0))
    eq_cond = np.concatenate((eq_cond, var[0:7] - (v_init[0], chi_init[0], gamma_init[0], teta_init[0],
                                                       lam_init[0], h_init[0], m_init[0])))
    eq_cond = np.concatenate(
        (eq_cond, var[Nstates:varStates] - conj[:Nstates * (Nleg - 1)]))  # knotting conditions

    eq_cond = np.concatenate((eq_cond, (var[varStates] - alfa_toNew(0.0),)))  # init cond on alpha
    eq_cond = np.concatenate((eq_cond, (var[varStates + 1] - 1.0,)))  # init cond on delta
    eq_cond = np.concatenate((eq_cond, (var[varStates + 2] - deltaf_toNew(0.0),)))  # init cond on deltaf
    eq_cond = np.concatenate((eq_cond, (var[varStates + 3] - tau_toNew(0.0),)))  # init cond on tau
    eq_cond = np.concatenate((eq_cond, (var[varStates + 4] - mu_toNew(0.0),)))  # init cond on mu
    eq_cond = np.concatenate((eq_cond, (v_toNew(vvv) - v_toNew(vtAbs),)))
    eq_cond = np.concatenate((eq_cond, (chi_toNew(chifin) - chi_toNew(chiass),)))
    eq_cond = np.concatenate((eq_cond, (conj[varStates - 5] - gamma_toNew(0.0),)))  # final condition on gamma
...
def constraints(var):
    ...
    if all(j<general_tol/10 for j in abs(var - obj.varOld)):
        return np.concatenate((obj.eqOld, obj.ineqOld))

    else:
        ineq_c, eq_c, cost = MultiShooting(var, dynamicsInt)
...
sparseJac = load_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/sparse_festip_5leg5contMuLess.npz")#
...
lb = lbeq * (Nstates*Nleg + 3 + Ncontrols) + lbineq * (4 * NineqCond * Nleg + 1) # all lower bounds
ub = ubeq * (Nstates*Nleg + 3 + Ncontrols) + ubineq * (4 * NineqCond * Nleg + 1) # all upper bounds
...
#save_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/sparse_festip_5leg15contMuLess.npz", sparse)
