...
varControls = Ncontrols * NContPoints * Nleg # Ncontrols * (Nleg * (NContPoints - 1) + 1)   # total number of optimization variables for controls
varTot = varStates + varControls  # total number of optimization variables for states and controls
NineqCond = Nleg * NContPoints - Nbar + 2
...
def inequalityAll(states, controls, varnum):
    ...
    iC = np.hstack((axnewlim, aznewlim, qnewlim, Momnewlim, mnew))
...
def MultiShooting(var, dyn):
    ...
    ineq_Cond = np.asarray(ineq_Cond, dtype=np.float64)
    eq_Cond = np.asarray(eq_Cond, dtype=np.float64)
...
def equality(var, conj):
    ...
    eq_cond = np.concatenate((eq_cond, (conj[varStates - 5]/1e3 - gamma_toNew(0.0),)))  # final condition on gamma
...
'''NLP SOLVER'''
n = np.int((len(X0)))
opt = nlopt.opt(nlopt.LD_SLSQP, 129)
opt.set_min_objective(cost_fun)
#opt.set_lower_bounds(lb)
#opt.set_upper_bounds(ub)
#opt.add_inequality_constraint(lambda var, grad: constraints(var, grad, "ineq"))
#opt.add_equality_constraint(lambda var, grad: constraints(var, grad, "eq"))
