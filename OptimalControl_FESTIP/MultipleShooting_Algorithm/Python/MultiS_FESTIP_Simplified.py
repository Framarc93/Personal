...
'''this script is a multiple shooting algorithm applied on a simplified version of FESTIP. 
Semplifications applied:
STATES: v, chi, gamma, teta, lambda, h, m
CONTROLS: alfa, delta, deltaf, tau
removed constraint on pitching moment
start from the equator and moving straightforward
...
class Spaceplane:
    ...
    def __init__(self):
        ...
        self.Re = 6378000  # Earth Radius [m]
        self.psl = 101325  # ambient pressure at sea level [Pa]
        self.latstart = 0  # deg latitude
        self.longstart = 0  # deg longitude
        self.chistart = 180 # deg flight direction
        self.incl = 0  # deg orbit inclination
        self.gammastart = 89.9  # deg
        self.M0 = 450400  # kg  starting mass
        self.g0 = 9.80665  # m/s2
        self.gIsp = self.g0 * 455  # g0 * Isp max
        self.omega = 0.0 #7.2921159e-5
        self.MaxQ = 40000  # Pa
        self.MaxAx = 30  # m/s2
        self.MaxAz = 15  # m/s2
...
time_tot = 300  # s
Npoints = 10  # number of multiple shooting points
...
v_guess =np.array(([0.1, 3.35466889e+01, 3.55270005e+02, 1.26169097e+03, 2.94231538e+03, 4.94757217e+03, 6.13062730e+03, 6.78802329e+03, 7.46895240e+03, 7.85070852e+03]))
chi_guess = np.array(([113.0, 116.30423696, 116.55208878, 117.92354784, 121.41884321, 124.0532628, 126.4974523, 128.1499793, 128.58329859, 128.58791495]))
gamma_guess = np.array(([89.9, 83.78757497, 81.15171249, 69.06418848, 47.98137708, 28.98511628, 13.56998971, 3.28760848, 0.16768323, 0.0]))
teta_guess = np.array(([-52.775, -52.77498353, -52.77713044, -52.78909518, -52.85679498, -52.99701637, -53.04747186, -52.95654205, -52.89033306, -52.88833992]))
lam_guess = np.array(([5.2, 5.20007366, 5.20362965, 5.22300301, 5.30003444, 5.43300977, 5.44236503, 5.29294487, 5.1991067, 5.19589304]))
h_guess = np.array(([0.1, 6.91699584e+02, 6.88379667e+03, 2.35289454e+04, 4.60283074e+04, 6.68505779e+04, 8.31630775e+04, 9.23002789e+04, 9.57397295e+04, 9.62557344e+04]))
...
alfa_guess = np.array(([0.0, 3.2622134, 0.20332326, -1.29089827, -1.98241146, 5.64311896, -0.94849787, -1.99314296, 7.58186611, -1.89409715]))
delta_guess = np.array(([1.0, 1.0, 1.0, 1.0, 0.85892564, 0.42352122, 0.21, 0.19525226, 0.193, 0.18952055]))
...
'''
plt.figure()
plt.plot(tnew, v_interp(tnew), marker="o")
plt.figure()
plt.plot(tnew, chi_interp(tnew))
plt.figure()
plt.plot(tnew, gamma_interp(tnew))
plt.figure()
plt.plot(tnew, teta_interp(tnew))
plt.figure()
plt.plot(tnew, lam_interp(tnew))
plt.figure()
plt.plot(tnew, h_interp(tnew))
plt.figure()
plt.plot(tnew, m_interp(tnew))
plt.figure()
plt.plot(tnew, alfa_interp(tnew))
plt.figure()
plt.plot(tnew, delta_interp(tnew))
plt.show()
'''
...
XGuessOG = np.array((v_interp(tnew)/ unit_v, chi_interp(tnew)/ unit_chi, gamma_interp(tnew)/ unit_gamma,  # states initial guesses from OpenGoddard data
                   teta_interp(tnew)/ unit_teta, lam_interp(tnew), h_interp(tnew)/ unit_h, m_interp(tnew)/ unit_m))

UGuessOG = np.array((alfa_interp(tnew)/ unit_alfa, delta_interp(tnew)/ unit_delta))  # controls initial guesses from OpenGoddard data

'''definition of another set of initial guesses, more random but reasonable'''

v_init = Guess.cubic(tnew, 1.0, 0.0, obj.Vtarget, 0.0)
chi_init = Guess.constant(tnew, obj.chistart)
...
delta_init = Guess.cubic(tnew, 1.0, 0.0, 0.01, 0.0)
...
def dynamics(t, states, controls):
    ...
    dx = np.array((((T * np.cos(eps) - D) / m) - g * np.sin(gamma),
                - np.cos(gamma) * np.cos(chi) * np.tan(lam) * (v / (obj.Re + h)),
                ((T * np.sin(eps) + L) / (m * v)) - (g / v - v / (obj.Re + h)) * np.cos(gamma),
...
#bndX = ((0.001/unit_v, np.inf), (90/unit_chi, 270/unit_chi), (-np.inf, np.inf), (-np.inf, np.inf), (-np.inf, np.inf),
        #(0.001/unit_h, 190000/unit_h), ((obj.m10*1.05)/unit_m, (obj.M0)/unit_m))
bndX = ((1e-9/unit_v, np.inf), (-np.inf, np.inf), (0.0, np.inf), (-np.inf, np.inf),
        (-np.inf, np.inf), (1e-9/unit_h, np.inf), (obj.m10/unit_m, obj.M0/unit_m))
bndU = ((-2/unit_alfa, 40/unit_alfa), (0.001, 1.0), (-20/unit_deltaf, 30/unit_deltaf), (0.0, 1.0))
bndT = ((0.01, 0.3),)
...
init_condX = np.array((1/unit_v, obj.chistart/unit_chi, obj.gammastart/unit_gamma, 0.0, 0.0, 1/unit_h, obj.M0/unit_m))
...
def inequality(states, controls):
    ...
    q = 0.5 * rho * (v ** 2)
     ...
     #   print(q, rho, v, h)
    # accelerations
    ax = (T - D * np.cos(alfa) + L * np.sin(alfa)) / m
    az = (T + D * np.sin(alfa) + L * np.cos(alfa)) / m
    ...
    return [(obj.MaxAx-ax)/obj.MaxAx, (obj.MaxAz-az)/obj.MaxAz, (obj.MaxQ-q)/obj.MaxQ]
...
def ineqCond(var):
    ...
    #ineq_cond = np.concatenate((ineq_cond, (mf/unit_m-obj.m10/unit_m,)))
    #ineq_cond = np.concatenate((ineq_cond, (h/unit_h-80000/unit_h,)))
    #j=0
    #print(ineq_cond)
...
def propagation(var, dynamics):
    ...
    states_atNodes = np.zeros((varStates-7))  # this is the output of the states on the nodes.
    #states_atNodes[0:Nstates] = var[0:Nstates]  # The first node is initialized with the initial conditions scaled
    #plt.figure()
    ...
    for i in range(Npoints-1):
        ...
        states_atNodes[((i)*Nstates):(Nstates*(i+1))] = prop.y[:, -1]/states_unit  # the states at nodes vector is filled with scaled values
...
def equality(var):
    ...
    '''in eq constraints the reference comes after'''
    ...
    eq_cond = np.concatenate((eq_cond, var[Nstates:varStates]-conj))
    eq_cond = np.concatenate((eq_cond, var[varStates:varStates + 4] - init_condU))
    eq_cond = np.concatenate((eq_cond, (var[varStates-5]-final_cond,)))
...
print("Time elapsed:for total optimization ", end - start)
...
plt.step(time, delta, where='post', marker="o", label="Delta")
plt.step(time, tau, where='post', marker="o", label="Tau")
plt.grid()
plt.xlabel("time [s]")
plt.ylabel(" % ")
plt.legend(loc="best")
...
plt.step(time, np.rad2deg(alfa), where='post', marker="o", label="Alfa")
plt.grid()
plt.xlabel("time [s]")
plt.ylabel(" deg ")
plt.legend(loc="best")
...
plt.step(time, np.rad2deg(deltaf), where='post', marker="o", label="Delta f")
for line in time:
    plt.axvline(line, color="k", alpha=0.5)
plt.grid()
plt.xlabel("time [s]")
