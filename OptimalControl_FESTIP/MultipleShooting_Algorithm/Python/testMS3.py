...
UGuess = np.array((alfa_init / unit_alfa, delta_init / unit_delta, deltaf_init / unit_deltaf,
                   tau_init / unit_tau, mu_init/unit_mu))  # states initial guesses
...
Xlb = ([1e-5/unit_v, np.deg2rad(90)/unit_chi, np.deg2rad(-89.9)/unit_gamma, -np.inf, -obj.incl/unit_lam, 1e-5/unit_h, obj.m10/unit_m]) # states lower bounds
...
Ulb = ([np.deg2rad(-2)/unit_alfa, 0.001, np.deg2rad(-20)/unit_deltaf, -1.0, np.deg2rad(-90)/unit_mu]) # controls lower bounds
Uub = ([np.deg2rad(40)/unit_alfa, 1.0, np.deg2rad(30)/unit_deltaf, 1.0, np.deg2rad(90)/unit_mu]) # controls upper bounds

Tlb = ([0.2/unit_t,]) # time lower bounds
...
def inequalityAll(states, controls, varnum):
    ...
    iC = np.hstack(((obj.MaxAx - ax)/(obj.MaxAx*1000), (obj.MaxAz - az)/(obj.MaxAz*1000), (obj.MaxQ - q)/(obj.MaxQ*1000),
                      (obj.k - MomTot)/(obj.k*1000000), (MomTot + obj.k)/(obj.k*1000000), (mf - obj.m10)/unit_m,
                    (gamma + np.deg2rad(89.9))/unit_gamma, (np.deg2rad(89.9) - gamma)/unit_gamma, (chi - np.deg2rad(90))/unit_chi,
                    (np.deg2rad(270) - chi)/unit_chi))  # when with bounds
...
def SingleShooting(states, controls, dyn, tstart, tfin, Nint):
    ...
    mu_Int = interpolate.PchipInterpolator(Time, controls[4, :] * unit_mu)
    ...
    return vres, chires, gammares, tetares, lamres, hres, mres, time_new, alfares, deltares, deltafres, taures, mures, \
           alfa_Int, delta_Int, deltaf_Int, tau_Int, mu_Int
...
def MultiShootingFun(var, dyn):
    ...
    for i in range(Nleg):
        ...
        controls = np.vstack((alfa, delta, deltaf, tau, mu)) # scaled

        vres, chires, gammares, tetares, lamres, hres, mres, tres, alfares, deltares, deltafres, taures, mures, alfa_I, delta_I, deltaf_I, tau_I, mu_I = SingleShooting(
...
def MultiShootingCons(var, dyn):
    ...
    for i in range(Nleg):
        ...
        muineq[0, :] = mures  # tau_I(time_ineq)
        ...
        mut = np.transpose(muineq)

        if i == 0:
            states_after = np.column_stack((vt, chit, gammat, tetat, lamt, ht, mt))
            controls_after = np.column_stack((alfat, deltat, deltaft, taut, mut))
        else:
            states_after = np.vstack((states_after, np.column_stack((vt, chit, gammat, tetat, lamt, ht, mt))))
            controls_after = np.vstack((controls_after, np.column_stack((alfat, deltat, deltaft, taut, mut))))
...
def equality(var, conj):
    ...
    div = abs(conj[:Nstates * (Nleg - 1)]*np.repeat(states_unit, Nleg-1))
    if any(div) <= 0:
    ...
    eq_cond = np.concatenate((eq_cond, var[0:7] - (v_init[0] / unit_v, chi_init[0] / unit_chi, gamma_init[0] / unit_gamma, teta_init[0] / unit_teta,
    ...
    eq_cond = np.concatenate((eq_cond, (var[varStates],)))  # init cond on alpha
    eq_cond = np.concatenate((eq_cond, (var[varStates + 1] - 1.0,)))  # init cond on delta
    eq_cond = np.concatenate((eq_cond, (var[varStates + 2],)))  # init cond on deltaf
    eq_cond = np.concatenate((eq_cond, (var[varStates + 3],)))  # init cond on tau
    eq_cond = np.concatenate((eq_cond, (var[varStates + 4],)))  # init cond on mu
    eq_cond = np.concatenate((eq_cond, ((vvv - vtAbs) / unit_v,)))
    eq_cond = np.concatenate((eq_cond, ((chifin - chiass) / unit_chi,)))
...
#sparseJac = load_npz("/home/francesco/Desktop/PhD/FESTIP_Work/MultipleShooting_Algorithm/testMS3_6x7.npz")#
...
lb = lbeq * (Nstates*Nleg + 3 + Ncontrols) + lbineq * (9 * NineqCond * Nleg + 1) # all lower bounds
ub = ubeq * (Nstates*Nleg + 3 + Ncontrols) + ubineq * (9 * NineqCond * Nleg + 1)# all upper bounds
...
save_npz("/home/francesco/Desktop/PhD/FESTIP_Work/MultipleShooting_Algorithm/testMS3_5x7.npz", sparse)