...
function coeffFinal = coefCalc(coeff, m, alfa, deltaf)
        ...
        c11 = interp1([angAttack(ia), angAttack(sa)], coeffinf1, alfa); %interpolazione 1D
        c21 = interp1([angAttack(ia), angAttack(sa)], coeffsup1, alfa); %interpolazione 1D

        coeffd1 = interp1([bodyFlap(id), bodyFlap(sd)], [c11, c21],deltaf); %?
        ...
        c12 = interp1([angAttack(ia), angAttack(sa)], coeffinf2, alfa); %interpolazione 1D
        c22 = interp1([angAttack(ia), angAttack(sa)], coeffsup2, alfa); %interpolazione 1D
        coeffd2 = interp1([bodyFlap(id), bodyFlap(sd)], [c12, c22],deltaf);

        %%%%%%%%%%%%%%%

        coeffFinal = interp1([mach(im), mach(sm)],[coeffd1, coeffd2],m);
    end
