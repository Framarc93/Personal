function [vres, chires, gammares, tetares, lamres, hres, mres, t, alfares, deltares, deltafres, taures] = SingleShooting(states, controls, timestart, timeend, prob, obj, file)
...
timeCont = linspace(timestart, timeend, length(controls(1,:)));
...
x(1,:) = states;
for i=1:prob.Nint-1
    k1 = dt*dynamicsInt(t(i), x(i, :), alfa_Int, delta_Int, deltaf_Int, tau_Int, obj, file);
    k2 = dt*dynamicsInt(t(i) + dt / 2, x(i, :) + k1 / 2, alfa_Int, delta_Int, deltaf_Int, tau_Int, obj, file);
    k3 = dt*dynamicsInt(t(i) + dt / 2, x(i, :) + k2 / 2, alfa_Int, delta_Int, deltaf_Int, tau_Int, obj, file);
    k4 = dt*dynamicsInt(t(i + 1), x(i, :) + k3, alfa_Int, delta_Int, deltaf_Int, tau_Int, obj, file);
