function [press, dens, sspeed] = isa_FESTIP(altitude, obj)
    
if isnan(altitude)
    altitude = 1;
else
    if isinf(altitude)
        altitude = obj.hmax;
    end
end
    a = [-0.0065, 0, 0.0010, 0.0028, 0, -0.0020, -0.0040, 0];
    a90 = [0.0030, 0.0050, 0.0100, 0.0200, 0.0150, 0.0100, 0.0070];
    hv = [11000, 20000, 32000, 47000, 52000, 61000, 79000, 90000];
    h90 = [90000, 100000, 110000, 120000, 150000, 160000, 170000, 190000];
    tmcoeff = [180.65, 210.65, 260.65, 360.65, 960.65, 1110.65, 1210.65];
    pcoeff = [0.16439, 0.030072, 0.0073526, 0.0025207, 0.505861E-3, 0.36918E-3, 0.27906E-3];
    tcoeff2 = [2.937, 4.698, 9.249, 18.11, 12.941, 8.12, 5.1];
    tcoeff1 = [180.65, 210.02, 257.0, 349.49, 892.79, 1022.2, 1103.4];
    p0 = obj.psl;
    t0 = 288.15;
    prevh = 0.0;
    g0 = obj.g0;
    R = 287.00;
    m0 = 28.9644;
    Rs = 8314.32;
    r = obj.Re;
   
    if altitude < 0 || isnan(altitude)
        temp = t0;
        press = p0;
        dens = press / (R * temp);
        sspeed = sqrt(1.4 * R * temp);
    end
    if (ge(altitude,0) && altitude < 90000) 
        for i=1:8
            if (le(altitude,hv(i)))
                [temp,press]=cal(p0,t0,a(i),prevh,altitude, obj);
                dens = press / (R * temp);
                sspeed = sqrt(1.4 * R * temp);
                break
            else
                [t0, p0] = cal(p0, t0, a(i), prevh, hv(i), obj);
                prevh = hv(i);
            end
        end
    end
    
    if (ge(altitude,90000) && le(altitude,190000))
            [temp, press, tpm] = atm90(a90, altitude, h90, tcoeff1, pcoeff, tcoeff2, tmcoeff);
            dens = press / (R * temp);
            sspeed = sqrt(1.4 * R * tpm);
    end
    
    if (altitude > 190000) || isinf(altitude)
       zb = h90(7);
       z = h90(end);
       b = zb - tcoeff1(7) / a90(7);
       temp = tcoeff1(7) + (tcoeff2(7) * (z - zb)) / 1000;
       tm = tmcoeff(7) + a90(7) * (z - zb) / 1000;
       add1 = 1 / ((r + b) * (r + z)) + (1 / ((r + b) ^ 2)) * log(abs((z - b) / (z + r)));
       add2 = 1 / ((r + b) * (r + zb)) + (1 / ((r + b) ^ 2)) * log(abs((zb - b) / (zb + r)));
       press = pcoeff(7) * exp(-m0 / (a90(7) * Rs) * g0 * r ^ 2 * (add1 - add2));
       dens = press / (R * temp);
       sspeed = sqrt(1.4 * R * tm);
    end
    

 function [t1, p1] = cal(ps, ts, av, h0, h1, obj)
      
    t0 = 288.15;
    p0 = obj.psl;
    prevh = 0.0;
    R = 287.00;
    m0 = 28.9644;
    Rs = 8314.32;
    m0 = 28.9644;
    r = obj.Re;
    g0 = obj.g0;
    
            if (av ~= 0)
                t1 = ts + av * (h1 - h0);
                p1 = ps * (t1 / ts) ^ (-g0 / av / R);
            else
                t1 = ts;
                p1 = ps * exp(-g0 / R / ts * (h1 - h0));
            end
    end
    
 function [temp,press,tm] = atm90(a90v, z ,hi, tc1, pc, tc2, tmc)
    t0 = 288.15;
    p0 = 101325;
    prevh = 0.0;
    R = 287.00;
    m0 = 28.9644;
    Rs = 8314.32;
    m0 = 28.9644;
    r = 6371000;
    g0 = 9.80665;
    ni=1;
        while (le(ni,length(hi)))
            if le(z,hi(ni))
                         ind=ni;
                         if (ind == 1)
                            zb = hi(1);
                            b = zb - tc1(1) / a90v(1);
                            temp = tc1(1) + tc2(1) * (z - zb) / 1000;
                            tm = tmc(1) + a90v(1) * (z - zb) / 1000;
                            add1 = 1 / ((r + b) * (r + z)) + (1 / ((r + b)^ 2)) * log(abs((z - b) / (z + r)));
                            add2 = 1 / ((r + b) * (r + zb)) + (1 / ((r + b)^ 2)) * log(abs((zb - b) / (zb + r)));
                            press = pc(1) * exp(-m0 / (a90v(1) * Rs) * g0 * r ^ 2 * (add1 - add2));
                         else
                            zb = hi(ind-1);
                            b = zb - tc1(ind-1) / a90v(ind-1);
                            temp = tc1(ind-1) + (tc2(ind-1) * (z - zb)) / 1000;
                            tm = tmc(ind - 1) + a90v(ind - 1) * (z - zb) / 1000;
                            add1 = 1 / ((r + b) * (r + z)) + (1 / ((r + b) ^ 2)) * log(abs((z - b) / (z + r)));
                            add2 = 1 / ((r + b) * (r + zb)) + (1 / ((r + b) ^ 2)) * log(abs((zb - b) / (zb + r)));
                            press = pc(ind - 1) * exp(-m0 / (a90v(ind - 1) * Rs) * g0 * r ^ 2 * (add1 - add2));
                           break
                         end
            end
            ni=ni+1;
        end
 end    
end

