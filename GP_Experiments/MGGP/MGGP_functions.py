...
def optimize_ind(ind, evaluate, Ngenes):
    ...
    for i in range(len(ind)):
        for j in range(len(ind[i])):
            if type(ind[i][j]) == gp.Terminal and ind[i][j].name[0] != "A":
                ind[i][j] = deepcopy(ind[i][j])
                ind[i][j].value = float(lst.x[c + Ngenes + 1])
                ind[i][j].name = str(lst.x[c + Ngenes + 1])
                c += 1
...
def lst(ind, evaluate_lst, output):

    d = ind.w
    #try:
    lst = least_squares(evaluate_lst, d, args=(ind,), method='lm', loss='linear')
    ind.w = lst.x  # update the linear combination weights after optmization
    fit = 1/np.std(output) * np.sqrt(1/len(lst.fun) * sum((lst.fun) ** 2))  # output-output_eval
    ind.fitness.values = fit,
    #except(ValueError, TypeError):
     #   ind.fitness.values = 1e5,
    return ind