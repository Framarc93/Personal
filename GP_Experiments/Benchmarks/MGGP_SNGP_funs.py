...
class Combination(object):
    ...
    def update(self, pop, connections, weights, evaluate_ind, ref_fun, interval, terminals):
        self.clear()
        ...
        expr = self.build_eq()
        fit = evaluate_ind(expr, ref_fun, interval, terminals)
        self.fitness = fit[0]
        self.equation = expr
...
def multi_mutation(count, comb_pop, pop, nElem_max, evaluate_lst, evaluate_ind, ref_fun, interval, terminals):
    #best_cpop = shuffle(tools.selBest(comb_pop, int(len(comb_pop)/2)))
    start_comb = deepcopy(comb_pop[0]) #deepcopy(random.choice(comb_pop))
    new_comb = mutate_combination(start_comb, pop, unif=0.3, shr=0.4, nElem_max=nElem_max, evaluate_ind=evaluate_ind, ref_fun=ref_fun, interval=interval, terminals=terminals)
    w = []
    ...
    if len(d) != 0:
       #res = minimize(evaluate, d, args=(new_comb, ref_fun, interval), method='BFGS', options={'disp': False})
        res = least_squares(evaluate_lst, d, args=(new_comb, ref_fun, interval, terminals), method='lm', loss='linear')

    return [new_comb, res.x]

def multi_cross(count, hof_comb, pop, evaluate_ind, evaluate):
    best_cpop = shuffle(tools.selBest(hof_comb, int(len(hof_comb)/2)))
    ch1 = deepcopy(random.choice(best_cpop))
    ch2 = deepcopy(random.choice(best_cpop))
    while ch1 == ch2:
        ch2 = deepcopy(random.choice(best_cpop))
...
def mutate_combination(combination, pop, unif, shr, nElem_max, evaluate_ind, ref_fun, interval, terminals):
    ch = random.random()
    if ch <= unif:
        combination = mutate_uniform(combination, pop, nElem_max, evaluate_ind, ref_fun, interval, terminals)
    elif ch <= unif + shr and len(combination.connections)>1:
        combination = mut_shrink(combination, pop, evaluate_ind, ref_fun, interval, terminals)
    else:
        combination = mut_replacement(combination, pop, evaluate_ind, ref_fun, interval, terminals)
    return combination

def mutate_uniform(combination, pop, nElem_max, evaluate_ind, ref_fun, interval, terminals):
    ...
    indexes = tools.selBest(pop, int(len(pop)/4))  # select the best individuals in the population
    nodes_list = []
    ...
    weights[0] = 0.0
    combination.update(pop, new_conn, weights, evaluate_ind, ref_fun, interval, terminals)

    return combination

def mut_replacement(combination, pop, evaluate_ind, ref_fun, interval, terminals):
    ...
    inds_list = tools.selBest(pop, int(len(pop)/4))
    nodes_list = []
    for i in range(len(pop)):
    ...
    for i in range(len(combination.connections)):
        if combination.connections[i] in chosen_nodes:
            ...
            nn += 1
    combination.update(pop, combination.connections, combination.weights, evaluate_ind, ref_fun, interval, terminals)
    return combination

def mut_shrink(combination, pop, evaluate_ind, ref_fun, interval, terminals):
    to_remove = random.randint(0, len(combination.connections) - 2)
    if to_remove > 0:
        while to_remove > 0:
...
def create_combinations(pop, nElem_max, evaluate_ind, ref_fun, interval, terminals):
    ...
    weights[0] = 0.0
    combinations.update(pop, connections, weights, evaluate_ind=evaluate_ind, ref_fun=ref_fun, interval=interval, terminals=terminals)
    return combinations

def update_comb_afterOpt(pop, comb, d, toolbox, evaluate_ind, ref_fun, interval, terminals):
    ...
    for ind, fit in zip(invalid_ind, fitnesses):
        ind.fitness.values = fit
    up_comb.update(pop_new, up_comb.connections, up_comb.weights, evaluate_ind, ref_fun, interval, terminals)
