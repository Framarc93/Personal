...
This script contains the functions used by MGGP
...
'''def optimize_ind(ind, evaluate, Ngenes, ref_fun, interval):
    ...
    try:
        lst = minimize(evaluate, d, args=(ind, ref_fun, interval), method='BFGS')
...
def lst(ind, evaluate_lst, f1, f2):
    d = ind.w
    #try:
    lst = least_squares(evaluate_lst, d, args=(ind, f1, f2))
...
def build_eq(individual):
    eq = str(individual.w[0]) + "+"
    i = 1
    while i < len(individual):
        eq = eq + str(individual.w[i]) + "*" + str(individual[i-1]) + "+"
        i += 1
    eq = list(eq)
    del eq[-1]
    eq = "".join(eq)
