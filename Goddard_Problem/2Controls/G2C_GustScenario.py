...
intensity acting in a random altitude range is simulated  
...
flag_save = True
if flag_save:
    os.makedirs("/home/francesco/Desktop/Git_workspace/IC4A2S/Goddard_Problem/Results/2CGust_Learning_1000it_Res_{}_{}".format(
        os.path.basename(__file__), timestr))
    savefig_file = "/home/francesco/Desktop/Git_workspace/IC4A2S/Goddard_Problem/Results/2CGust_Learning_1000it_Res_{}_{}/Plot_".format(
        os.path.basename(__file__), timestr)
    savedata_file = "/home/francesco/Desktop/Git_workspace/IC4A2S/Goddard_Problem/Results/2CGust_Learning_1000it_Res_{}_{}/".format(
        os.path.basename(__file__), timestr)
...
def main():
    ...
    pop, log = eaMuPlusLambdaTol(best_pop, toolbox, Mu, Lambda, size_gen, 1.5, stats=mstats, halloffame=hof,
                                 verbose=True)
    ####################################################################################################################
...
old_hof = HallOfFame(int(size_pop_tot/1.5))
...
while nt < ntot:
    ...
    if flag_save:
        output = open(
            "/home/francesco/Desktop/Git_workspace/IC4A2S/Goddard_Problem/Results/2CGust_Learning_1000it_Res_{}_{}/hof_Offline{}.pkl".format(
    ...
    #########for plot and to find initial condition for integration#########
    ...
    for i in range(len(t_eval_p)):
        if t_eval_p[i] >= change_time + delta_eval:
            ...
            break
    #####  integration  #####
    #x_ini2 = [r_eval_p[index], th_eval_p[index], vr_eval_p[index], vt_eval_p[index], m_eval_p[index]]
    tev = np.linspace(change_time + delta_eval, tfin, 1000)
    solgp = solve_ivp(partial(sys2GP, expr1=hof[-1][0], expr2=hof[-1][1], v_wind=v_wind), [change_time + delta_eval, tfin], x_ini_h, t_eval=tev)  # used for next integration
    ...
    plt.figure(3)
    plt.plot(t_eval_p, vt_eval_p, color='b')
    plt.plot(ttgp, vtout,label="V wind={} m/s, Start gust={} m, End gust={} m".format(round(v_wind, 2), round(height_start-obj.Re,2), round(height_start+delta-obj.Re)))
    plt.xlabel("time [s]")
    ...
    plt.plot(t_eval_p, m_eval_p, color='b')
    plt.plot(ttgp, mout,label="V wind={} m/s, Start gust={} m, End gust={} m".format(round(v_wind, 2), round(height_start-obj.Re,2), round(height_start+delta-obj.Re)))
    plt.xlabel("time [s]")
    plt.ylabel("Mass [kg]")
    plt.legend(loc='best')

    plt.figure(5)
    plt.plot(t_eval_p, vr_eval_p, color='b')
    plt.plot(ttgp, vrout,label="V wind={} m/s, Start gust={} m, End gust={} m".format(round(v_wind, 2), round(height_start-obj.Re,2), round(height_start+delta-obj.Re)))
    plt.xlabel("time [s]")
