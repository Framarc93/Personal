'''G2C_Cd_GPwOpt: intelligent control of goddard rocket 2 control with local optimizer in the loop
...
def evaluate_opt(d, individual):
    global Rfun, Thetafun, Vrfun, Vtfun, mfun, Trfun, Ttfun, change_time, tfin
    ...
    for i in range(len(individual)):
        for j in range(len(individual[i])):
            if type(individual[i][j]) == gp.Terminal and individual[i][j].name[0] != "A":
                ...
                individual[i][j].value = float(d[c])
                individual[i][j].name = str(d[c])
                c += 1
...
def optimize_ind(ind):
    d = []
    old_ind = deepcopy(ind)
    for j in range(len(ind)):
        for h in range(len(ind[j])):
            if type(ind[j][h]) == gp.Terminal and ind[j][h].name[0] != "A":
                d.append(ind[j][h].value)
    if len(d) != 0:
        res = minimize(evaluate_opt, d, args=(ind,), method='BFGS', options={'disp':True})
    else:
        return ind
    c = 0
    for j in range(len(ind)):
        for h in range(len(ind[j])):
            if type(ind[j][h]) == gp.Terminal and ind[j][h].name[0] != "A":
                ind[j][h] = deepcopy(ind[j][h])
                ind[j][h].value = float(res.x[c])
                ind[j][h].name = str(res.x[c])
                c += 1

    ind.fitness.values = (res.fun, old_ind.fitness.values[1])
    ind.optimized = True
    return ind


def optimize_best(pop):
    best_feas = 1e5
    best_abs = 1e5
    i_f = 0
    i_b = 0
    for i in range(len(pop)):
        if pop[i].fitness.values[-1] == 0 and pop[i].fitness.values[0] < best_feas and pop[i].optimized is False:
            best_feas = pop[i].fitness.values[0]
            i_f = i
            pop[i].optimized = True
        elif pop[i].fitness.values[-1] != 0 and pop[i].fitness.values[-1] < best_abs and pop[i].optimized is False:
            best_abs = pop[i].fitness.values[-1]
            i_b = i
            pop[i].optimized = True
    ...
    if best_feas != 1e5:
        ...
        for j in range(len(pop[i_f])):
            for h in range(len(pop[i_f][j])):
                if type(pop[i_f][j][h]) == gp.Terminal and pop[i_f][j][h].name[0] != "A":
                    d.append(pop[i_f][j][h].value)

        if len(d) != 0:
            res = minimize(evaluate_opt, d, args=(pop[i_f],), method='BFGS', options={'disp':True})
        else:
            return old_pop
        ...
        for j in range(len(pop[i_f])):
            for h in range(len(pop[i_f][j])):
                if type(pop[i_f][j][h]) == gp.Terminal and pop[i_f][j][h].name[0] != "A":
                    pop[i_f][j][h] = deepcopy(pop[i_f][j][h])
                    pop[i_f][j][h].value = float(res.x[c])
                    pop[i_f][j][h].name = str(res.x[c])
                    c += 1
        pop[i_f].fitness.values = (res.fun, old_pop[i_f].fitness.values[1])
        return pop

    else:
        #d = []
        #old_pop = deepcopy(pop)
        #for j in range(len(pop[i_b])):
        #    for h in range(len(pop[i_b][j])):
         #       if type(pop[i_b][j][h]) == gp.Terminal and pop[i_b][j][h].name[0] != "A":
          #          d.append(pop[i_b][j][h].value)
        #if len(d) != 0:
        #    res = minimize(evaluate_opt, d, args=(pop[i_b],), method='BFGS')
        #else:
        #    return old_pop
        #c = 0
        #for j in range(len(pop[i_b])):
            #for h in range(len(pop[i_b][j])):
            #    if type(pop[i_b][j][h]) == gp.Terminal and pop[i_b][j][h].name[0] != "A":
            #        # if 'rand' in str(type(pop[i_f][j][h])):
            #        pop[i_b][j][h] = deepcopy(pop[i_b][j][h])
            #        pop[i_b][j][h].value = float(res.x[c])
            #        pop[i_b][j][h].name = str(res.x[c])
            #        c += 1
        #pop[i_b].fitness.values = (res.fun, old_pop[i_b].fitness.values[1])
        return pop
...
def InclusiveTournament(individuals, mu, to_mate):
    ...
    #for i in good_indexes:
     #   bb = selBest(organized_pop["cat{}".format(i)])
      #  organized_pop["cat{}".format(i)].remove(bb)
       # bb = optimize_best(bb)
        #organized_pop["cat{}".format(i)].append(bb)
    while len(chosen) < mu:
        ...
        if exploited[j] < len(organized_pop["cat{}".format(i)]):
            selected = selDoubleTournament(organized_pop["cat{}".format(i)], k=1, fitness_size=2, parsimony_size=1.4, enough=enough, fitness_first=True)
...
def subset_diversity(population):
    cat_number = 5  # here the number of categories is selected
    lens = []
    categories = {}
    distribution = []
    distr_stats = {}
...
def varOr(population, toolbox, lambda_, box, sub_div, good_indexes_original, tol):
    ...
    for _ in range(lambda_):
        ...
        if op_choice < cxpb:  # Apply crossover
            ind1, ind2 = deepcopy(map(toolbox.clone, [selBest(sub_div["cat{}".format(int(cat[0]))]), random.choice(sub_div["cat{}".format(int(cat[1]))])]))  # select one individual from the best and one from the worst
            ...
            #print("cross 1")
            #ind1 = optimize_ind(ind1)
            #if ind1.fitness.values[0] <= tol:
            del ind1.fitness.values
            offspring.append(ind1)
            #break
            #else:
            #print("cross 2")
            #ind2 = optimize_ind(ind2)
            #if ind2.fitness.values[0] <= tol:
            ...
            rr = random.random()
            if rr < 0.6:
                ind = deepcopy(toolbox.clone(selBest(population)))  # maybe better to use a sort of roulette
            else:
                ind = deepcopy(toolbox.clone(random.choice(population)))
            old = ind
            ind, = toolbox.mutate(ind)
            if sum(old.fitness.values) <= sum(ind.fitness.values) and len(ind) > limit_size:
                ind = old
            if ind.fitness.values[0] <= tol:
                del ind.fitness.values
                offspring.append(ind)
                break
        else:
            if len_subpop >= 1:
                chosen = random.choice(sub_pop)
            else:
                chosen = random.choice(selBest(population))
...
def eaMuPlusLambdaTol(population, toolbox, mu, lambda_, ngen, tol, stats=None, halloffame=None, verbose=__debug__):
    ...
    print("optimize pop")
    population = optimize_best(population)
    ...
    while gen < ngen + 1 and (min_fit[-1][0] > tol or min_fit[-1][-1] > 0.0):
        ...
        offspring, len_feas = varOr(population, toolbox, mu, halloffame, sub_div, good_index, tol)
        ...
        for ind, fit in zip(invalid_ind, fitnesses):
            ind.fitness.values = fit
        print("optimize offspring")
        offspring = optimize_best(offspring)
        # Update the hall of fame with the generated individuals
        if halloffame is not None:
            halloffame.update(offspring, for_feasible=True)

        # Select the next generation population
        population[:] = offspring[:] #toolbox.select(population + offspring, mu, to_mate=False)
...
def xmate(ind1, ind2):
    ...
    ind1.optimized = False
    ind2.optimzied = False
    return ind1, ind2
...
limit_height = 5  # Max height (complexity) of the controller law
...
def main():
    ...
    pop, log = eaMuPlusLambdaTol(pop, toolbox, Mu, Lambda, size_gen, 0.025, stats=mstats, halloffame=hof,
                                 verbose=True)
    ####################################################################################################################
...
def evaluate(individual):
    ...
    fitness1 = np.sqrt(sum((err1)**2))  # np.sqrt(sum(err1**2)) #+ sum(err3**2))#simps(abs(err1), tt)
    fitness2 = np.sqrt(sum((err2)**2))  # np.sqrt(sum(err2**2))# + sum(err4**2))#simps(abs(err2), tt)
...
for i in range(nEph):
    #psetT.addEphemeralConstant("randT{}".format(i), lambda: round(random.uniform(-5, 5), 4))
    psetT.addTerminal(round(random.uniform(-5, 5), 4))
...
creator.create("Individual", list, fitness=creator.Fitness, optimized=False)
...
toolbox.register("mutate", xmut, expr=toolbox.expr_mut, unipb=0.5, shrpb=0.2, inspb=0.15)  ### NEW ###
...
while nt < ntot:
    ...
    size_pop = 60 #- len(old_hof) # Pop size
    ...
    #b_hof = hof[-1]
    #hof.remove(-1)
    #b_hof = optimize_best(b_hof)
    #hof.insert(b_hof, for_feasible=True)
    ...
    nt += 1
print("Time success {}".format(round(success_time/ntot*100,2)))
print("Range success {}".format(round(success_range/ntot*100,2)))
print("Total success {}".format(round(success_range_time/ntot*100,2)))
