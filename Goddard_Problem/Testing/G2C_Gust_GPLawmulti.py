...
def evaluate(individual):
    ...
    def sys(t, x, height_start, delta, v_wind):
        global penalty, flag, flag_offdesign, flag_thrust, change_time
        # State Variables
        R = x[0]
        theta = x[1]
        Vr = x[2]
    ...
    c = 1
    while c < len(cases):
        sol = solve_ivp(partial(sys, height_start=cases[c][0], delta=cases[c][1], v_wind=cases[c][2]), [0, tfin], x_ini)
        y1 = sol.y[0, :]
        y2 = sol.y[1, :]
        tt = sol.t
...
toolbox.register("mutate", xmut, expr=toolbox.expr_mut, unipb=0.5, shrpb=0.25)  ### NEW ###
...
s = 0
delta_eval = 100
size_pop = 400 - len(old_hof) # Pop size
...
cases.append(["Height start [km]", "Size [km]", "Wind Speed [m/s]"])
...
it = 0
while it<100:
    t_change = random.uniform(20, 250)
    Cd_new = random.uniform(0.61, 2)
    x_ini = [obj.Re, 0.0, 0.0, 0.0, obj.M0]
    res = solve_ivp(partial(sys2GP_Cd, expr1=hof[-1][0], expr2=hof[-1][1], Cd_new=Cd_new, t_change=t_change), [0, tfin], x_ini)
    if Rref[-1]*0.99 < res.y[0, -1] < Rref[-1]*1.01 and Thetaref[-1]*0.99 < res.y[1, -1] < Thetaref[-1]*1.01:  # tolerance of 1%
        s += 1
    it += 1
...
output = open("hof_G2C_forNN.pkl".format(os.path.basename(__file__), timestr), "wb")  # save of hall of fame after first GP run
