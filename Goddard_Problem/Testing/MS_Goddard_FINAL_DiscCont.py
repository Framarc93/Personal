'''Multiple Shooting Optimal Control for open goddard 2 controls
...
maxiter = 25 # max number of iterations for nlp solver
ftol = 1e-8  # numeric tolerance of nlp solver
eps = 1e-10
Nstates = 5  # number of states
Ncontrols = 2  # number of controls
maxIterator = 50  # max number of optimization iterations
varStates = Nstates * Nleg
varControls = Ncontrols * Nleg * NContPoints  # total number of states variables
...
def MultiShooting(var, dynamics):
    ...
    for i in range(Nleg):
        ...
        for k in range(NContPoints):
            Tr[k] = var[varStates + i * (Ncontrols * NContPoints) + Ncontrols * k] * unit_T
            Tt[k] = var[varStates + i * (Ncontrols * NContPoints) + 1 + Ncontrols * k] * unit_T
...
def MultiPlot(var, dynamics, Nint):
    ...
    for i in range(Nleg):
        ...
        plt.figure(4)
        plt.title("Tr")
        plt.plot(tres, Trres/1000)
        plt.plot(tC, TrContPoint[i, :]/1000, 'ro')
        plt.axvline(tres[-1], color="k", alpha=0.5)
        plt.xlabel("time [s]")
        plt.ylabel("Tr [kN]")
        ...
        plt.figure(5)
        plt.title("Tt")
        plt.plot(tres, Ttres/1000)
        plt.plot(tC, TtContPoint[i, :]/1000, 'ro')
        plt.xlabel("time [s]")
        plt.ylabel("Tt [kN]")
        plt.axvline(tres[-1], color="k", alpha=0.5)
...
def equality(var, conj):
    ...
    eq_cond = np.concatenate((eq_cond, var[Nstates:varStates] - conj[:Nstates*(Nleg-1)]))
...
def JacFunSave(var):
    ...
    save_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/jacCostGod", sparse)
...
def JacEqSave(var, type):
    ...
    for i in range(len(x0)):
        ...
        dx[i] = 0.0
    sparse = csc_matrix(jac)
    save_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/jacEqGod", sparse)
    return jac.transpose()
...
def JacIneqSave(var, type):
    ...
    for i in range(len(x0)):
        ...
        dx[i] = 0.0
    sparse = csc_matrix(jac)
    save_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/jacIneqGod", sparse)
    return jac.transpose()
...
def JacFun(var):
    ...
    jac = np.zeros([len(x0), len(f0)])
    dx = np.zeros(len(x0))
    sparseJac = load_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/jacCostGod.npz")
...
def JacEq(var, type):
    ...
    jac = np.zeros([len(x0), len(f0)])
    dx = np.zeros(len(x0))
    sparseJac = load_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/jacEqGod.npz")
    sp = sparseJac.todense()
    row = np.shape(sp)[0]
    column = np.shape(sp)[1]
    for i in range(row):
...
def JacIneq(var, type):
    ...
    jac = np.zeros([len(x0), len(f0)])
    dx = np.zeros(len(x0))
    sparseJac = load_npz("/home/francesco/git_workspace/FESTIP_Work/MultipleShooting_Algorithm/jacIneqGod.npz")
    sp = sparseJac.todense()
    row = np.shape(sp)[0]
    column = np.shape(sp)[1]
    for i in range(row):
