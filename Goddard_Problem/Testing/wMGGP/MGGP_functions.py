...
def lst(ind, evaluate_lst, delta_eval, Cd_new, change_time, x_ini):
    d = []
    for i in range(len(ind)):
        d.extend(ind[i].w)
    #try:
    lst = least_squares(evaluate_lst, d, args=(ind, Cd_new, delta_eval, change_time, x_ini), method='lm', loss='linear', max_nfev=30)
...
def lstGust(ind, evaluate_lst, height_start, delta, change_time, x_ini):
    d = []
    for i in range(len(ind)):
        d.extend(ind[i].w)
    #try:
    lst = least_squares(evaluate_lst, d, args=(ind, height_start, delta, change_time, x_ini), method='lm', loss='linear')
    ind[0].w = lst.x[:len(ind[0].w)]
    ind[1].w = lst.x[len(ind[1].w):]# update the linear combination weights after optmization
    fit = np.sqrt(sum((lst.fun)**2))
    ind.fitness.values = fit,
...
def lstInversion(ind, evaluate_lst, delta_eval, Cd_new, change_time, x_ini):
    d = []
    for i in range(len(ind)):
        d.extend(ind[i].w)
    b, i1, i2, i3, i4 = evaluate_lst(d, ind, Cd_new, delta_eval, change_time, x_ini)
    A = np.ones((len(b), 1))
    A = np.column_stack((A, np.array(i1).reshape(len(b),1), np.array(i2).reshape(len(b),1), np.ones((len(b), 1)), np.array(i3).reshape(len(b),1), np.array(i4).reshape(len(b),1)))
