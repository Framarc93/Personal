...
flag_save = True
if flag_save:
    os.makedirs("/home/francesco/Desktop/Git_workspace/IC4A2S/Goddard_Problem/Results/2Control_Res_{}_{}".format(os.path.basename(__file__), timestr))
    savefig_file = "/home/francesco/Desktop/Git_workspace/IC4A2S/Goddard_Problem/Results/2Control_Res_{}_{}/Plot_".format(os.path.basename(__file__), timestr)
...
limit_size = 150  # Max size (complexity) of the controller law
...
def main():
    ...
    if flag_offdesign is False:
        ...
        fitnesses = toolx.map(toolbox.evaluate, pop)
    ...
    if flag_offdesign is True:
        pop, log = eaMuPlusLambdaTol(pop, toolx, Mu, Lambda, size_gen, 5, stats=mstats, halloffame=hof, verbose=True)  ### OLD ###
    else:
        pop, log = eaMuPlusLambdaTol(best_pop, toolbox, Mu, Lambda, size_gen, 5, stats=mstats, halloffame=hof, verbose=True)
    ####################################################################################################################
...
def evaluate(individual):
    ...
    if flag_offdesign is True:
        fTr = toolx.compileR(expr=individual[0])
        fTt = toolx.compileT(expr=individual[1])
    else:
    ...
    #IAE = np.zeros((2, len(err1)-1))
    #j = 0
    #alfa = 0.2
    #for a, b, c, d, e, n in zip(err1, err2, err3, err4, err5, tt):
      #  IAE[0][j] = abs(a)
     #   IAE[1][j] =
       # j = j + 1
    #for i in range(len(err1)-1):
    #    IAE[0][i] = abs(err1[i+1])*tt[i+1] - abs(err1[i])*tt[i]
    #    IAE[1][i] = abs(err2[i+1])*tt[i+1] - abs(err2[i])*tt[i]
...
psetR.addPrimitive(gpprim.posAdd, 2, name="Add")
psetR.addPrimitive(gpprim.posSub, 2, name="Sub")
psetR.addPrimitive(gpprim.posMul, 2)
psetR.addPrimitive(gpprim.posTriAdd, 3)
psetR.addPrimitive(gpprim.posTanh, 1, name="Tanh")
psetR.addPrimitive(gpprim.Sqrt, 1)
psetR.addPrimitive(gpprim.PosLog, 1)
psetR.addPrimitive(gpprim.ModExp, 1)
psetR.addPrimitive(gpprim.PosSin, 1)
psetR.addPrimitive(gpprim.PosCos, 1)

for i in range(nEph):
    psetR.addEphemeralConstant("randR{}".format(i), lambda: round(random.uniform(-10, 10), 4))
...
psetT.addPrimitive(gpprim.posAdd, 2, name="Add")
psetT.addPrimitive(gpprim.posSub, 2, name="Sub")
psetT.addPrimitive(gpprim.posMul, 2)
psetT.addPrimitive(gpprim.posTriAdd, 3)
psetT.addPrimitive(gpprim.posTanh, 1, name="Tanh")
psetT.addPrimitive(gpprim.Sqrt, 1)
psetT.addPrimitive(gpprim.PosLog, 1)
psetT.addPrimitive(gpprim.ModExp, 1)
psetT.addPrimitive(gpprim.PosSin, 1)
psetT.addPrimitive(gpprim.PosCos, 1)

for i in range(nEph):
    psetT.addEphemeralConstant("randT{}".format(i), lambda: round(random.uniform(-100, 10), 4))
...
solgp = solve_ivp(partial(sys2GP, Cd = obj.Cd, expr1=hof[-1][0], expr2=hof[-1][1]), [0.0, tfin], x_ini, t_eval=tevals)
print(solgp.message)
rout = solgp.y[0, :]
thetaout = solgp.y[1, :]
vrout = solgp.y[2, :]
...
plt.plot(ttgp, vtout, marker='.', color='k', label="ON DESIGN")
plt.figure(3)
...
plt.plot(ttgp, vrout, marker='.', color='k', label="ON DESIGN")
plt.figure(5)
plt.plot(tevals, tR, 'r--', label="SET POINT")
plt.plot(ttgp, thetaout, marker='.', color='k', label="ON DESIGN")

delta_time = 50 # estimated time to complete second GP run
i = 0
for i in range(len(ttgp)):
    if ttgp[i] >= change_time:
        ...
        break
for items in ttgp:
    if items >= change_time+delta_time:
        index_new = np.where(ttgp == items)
        break
...
if __name__ == "__main__":
    ...
    def extrapolate():
        ...
        if count < len(hof):
            ...
            res = tools.initIterate(creator.Individual2, toolx.legs2)
        return res
    count = int(len(hof)/2)


    creator.create("Fitness2", FitnessMulti, weights=(-1.0, -1.0))#, -0.1, -0.05, -1.0))  # MINIMIZATION OF THE FITNESS FUNCTION
    creator.create("Individual2", list, fitness=creator.Fitness2, height=1)
    creator.create("SubIndividual2", gp.PrimitiveTree, fitness=creator.Fitness2)
    toolx = base.Toolbox()

    #toolx.register("expr", gp.genHalfAndHalf, pset=pset, type_=pset.ret, min_=2, max_=5)  ### NEW ###

    toolx.register("legR2", tools.initIterate, creator.SubIndividual2, toolbox.exprR)  ### NEW ###
    toolx.register("legT2", tools.initIterate, creator.SubIndividual2, toolbox.exprT)  ### NEW ###
    toolx.register("legs2", tools.initCycle, list, [toolx.legR2, toolx.legT2], n=Ncontrols)  ### NEW ###
    #toolx.register("population", tools.initRepeat, list, extrapolate)

    toolx.register("individual", tools.initIterate, creator.Individual2, toolx.legs2)  ### NEW ###
    toolx.register("population", tools.initRepeat, list, toolbox.individual)
    toolx.register("compileR", gp.compile, pset=psetR)
    toolx.register("compileT", gp.compile, pset=psetT)

    toolx.register("evaluate", evaluate)  ### OLD ###
    toolx.register("select", InclusiveTournament)
    #toolx.register("select", tools.selNSGA2)  ### NEW ###
    #toolx.register("select", xselDoubleTournament, fitness_size=2, parsimony_size=1.2, fitness_first=True)  ### NEW ###
    toolx.register("mate", xmate)  ### NEW ###
    toolx.register("expr_mut", gp.genHalfAndHalf, min_=1, max_=4)  ### NEW ###

    toolx.register("mutate", xmut, expr=toolbox.expr_mut, unipb=0.5, shrpb=0.25, inspb=0.15)  ### NEW ###
    toolx.decorate("mate", staticLimit(key=operator.attrgetter("height"), max_value=limit_height))
    toolx.decorate("mutate", staticLimit(key=operator.attrgetter("height"), max_value=limit_height))

    toolx.decorate("mate", staticLimit(key=len, max_value=limit_size))
    toolx.decorate("mutate", staticLimit(key=len, max_value=limit_size))


    obj = Rocket()
    xnew_ini = [float(rout[index_new]), float(thetaout[index_new]), float(vrout[index_new]), float(vtout[index_new]), float(mout[index_new])]
    t_eval2 = np.linspace(change_time+delta_time, tfin, int(tfin*3))
    flag_seed_populations = True
    flag_offdesign = True
    flag_prop = False
    size_pop, size_gen, cxpb, mutpb = 300, 50, 0.8, 0.1
    pop, log, hof = main()
end = time()
t_offdesign = end - start  # CALCOLO TEMPO IMPIEGATO DAL GENETIC PROGRAMMING

if change_time + t_offdesign > tfin:
    t_offdesign = tfin - change_time
#######################################################################################################################
...
solgp_c = solve_ivp(partial(sys2GP, Cd=Cd_new, expr1=Tr_old, expr2=Tt_old) , [change_time, tfin], xnew_ini, t_eval=tevals_c)
...
plt.ion()
plt.figure(1)
plt.plot(ttgp_c, (rout_c - obj.Re) / 1e3, marker='.', color='b', label="OFF DESIGN")
plt.figure(2)
plt.plot(ttgp_c, vtout_c, marker='.', color='b', label="OFF DESIGN")
plt.figure(3)
plt.plot(ttgp_c, mout_c, marker='.', color='b', label="OFF DESIGN")
plt.figure(4)
plt.plot(ttgp_c, vrout_c, marker='.', color='b', label="OFF DESIGN")
plt.figure(5)
plt.plot(ttgp_c, thetaout_c, marker='.', color='b', label="OFF DESIGN")
...
flag_prop = False
solgp_gp = solve_ivp(partial(sys2GP, Cd=Cd_new, expr1=hof[-1][0], expr2=hof[-1][1]), [change_time + t_offdesign, tfin], xnew_ini_gp, t_eval=tevals_gp)
...
plt.ion()
plt.figure(1)
plt.plot(ttgp_gp, (rout_gp - obj.Re) / 1e3, marker='.', color='g', label="ONLINE CONTROL")
plt.xlabel("time [s]")
...
plt.figure(2)
plt.plot(ttgp_gp, vtout_gp, marker='.', color='g', label="ONLINE CONTROL")
plt.xlabel("time [s]")
plt.ylabel("Vt [m/s]")
plt.grid()
if flag_save:
...
plt.figure(3)
plt.plot(ttgp_gp, mout_gp, marker='.', color='g', label="ONLINE CONTROL")
plt.xlabel("time [s]")
plt.ylabel("mass [kg]")
plt.grid()
if flag_save:
...
plt.figure(4)
plt.plot(ttgp_gp, vrout_gp, marker='.', color='g', label="ONLINE CONTROL")
plt.xlabel("time [s]")
plt.ylabel("Vr [m/s]")
plt.grid()
if flag_save:
...
plt.figure(5)
plt.plot(ttgp_gp, thetaout_gp, marker='.', color='g', label="ONLINE CONTROL")
plt.xlabel("time [s]")
