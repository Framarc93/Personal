'''TreeTuner_G2C4NNTrain: script to tune a Gp Tree to face different conditions

This script takes a GP function generated by the script "G2C_Cd_GPLawmulti_hofNNGenerator.py" and test it on different
...
objects = []
with (open(initial_path + "/Goddard_Problem/Testing/NN_Control/hof_G2C_forNN_test.pkl", "rb")) as openfile:
    while True:
        try:
...
fTr = gp.compile(objects[0][-1][0], pset=psetR) # control law compiled from Gp tree
fTt = gp.compile(objects[0][-1][1], pset=psetT) # control law compiled from Gp tree
...
for i in range(len(objects[0][-1][0])):
    if type(objects[0][-1][0][i]) == gp.randR0 or type(objects[0][-1][0][i]) == gp.randR1:
        indexes.append(i)
        values.append(objects[0][-1][0][i].value)

for i in range(len(objects[0][-1][1])):
    if type(objects[0][-1][1][i]) == gp.randT0 or type(objects[0][-1][1][i]) == gp.randT1:
        indexes.append(i)
        values.append(objects[0][-1][1][i].value)
...
def test_param_opt(param, Cd_test, t_ch):
    ...
    for i in range(len(objects[0][-1][0])):
        try:
            if type(objects[0][-1][0][i]) == gp.randR0 or type(objects[0][-1][0][i]) == gp.randR1:
                objects[0][-1][0][i] = deepcopy(objects[0][-1][0][i])
                objects[0][-1][0][i].value = deepcopy(param[j])
                j += 1
    ...
    for i in range(len(objects[0][-1][1])):
        try:
            if type(objects[0][-1][1][i]) == gp.randT0 or type(objects[0][-1][1][i]) == gp.randT1:
                objects[0][-1][1][i] = deepcopy(objects[0][-1][1][i])
                objects[0][-1][1][i].value = deepcopy(param[j])
                j += 1
        except AttributeError:
            continue
    fTr = gp.compile(objects[0][-1][0], pset=psetR)
    fTt = gp.compile(objects[0][-1][1], pset=psetT)
    ...
    solgp = solve_ivp(sys2GP, [0, tfin], x_start, method='BDF')
    y1 = solgp.y[0, :]
    y2 = solgp.y[1, :]
    if solgp.t[-1] < tfin:
...
def tree_eval_opt(param, Cd_test, t_ch):
    ...
    x_start = [obj.Re, 0.0, 0.0, 0.0, obj.M0]
    solgp = solve_ivp(sys2GP, [0.0, tfin], x_start, method='BDF')
    y1 = solgp.y[0, :]
    y2 = solgp.y[1, :]
    tt = solgp.t
    ...
    #err2 = (theta - y2) / 0.9
    diffR = abs(r[-1] - y1[-1])/obj.Htarget
    diffTh = abs(theta[-1] - y2[-1])/0.9
    return diffR + diffTh
...
dataset = np.column_stack((solgp.t.reshape(len(solgp.t),1), solgp.y[0,:].reshape(len(solgp.t),1), solgp.y[1,:].reshape(len(solgp.t),1), solgp.y[2,:].reshape(len(solgp.t),1),
           solgp.y[3,:].reshape(len(solgp.t),1), (Rfun(solgp.t)-solgp.y[0,:]).reshape(len(solgp.t),1), (Thetafun(solgp.t)-solgp.y[1,:]).reshape(len(solgp.t),1),
...
def opti(n):
    ...
    if success is True:
        ...
        data = np.column_stack((res.t.reshape(len(res.t), 1), res.y[0, :].reshape(len(res.t), 1),
                                res.y[1, :].reshape(len(res.t), 1), res.y[2, :].reshape(len(res.t), 1),
                                res.y[3, :].reshape(len(res.t), 1), (Rfun(res.t) - res.y[0, :]).reshape(len(res.t), 1),
                                (Thetafun(res.t) - res.y[1, :]).reshape(len(res.t), 1),
                                (Vrfun(res.t) - res.y[2, :]).reshape(len(res.t), 1),
                                (Vtfun(res.t) - res.y[3, :]).reshape(len(res.t), 1)))
        for vv in best_val:
...
if __name__ == "__main__":
    nbCPU = mp.cpu_count()
    pool = mp.Pool(nbCPU)
    opt_res = pool.map(opti, range(3))
    dataset = []
    for i in range(len(opt_res)):
        if hasattr(opt_res[i], '__len__') and not dataset:
            dataset = opt_res[i]
        elif hasattr(opt_res[i], '__len__') and dataset:
            dataset = np.vstack((dataset, opt_res[i]))
'''while n < n_samples:
    ...
    if success is True:
        print("{} - Cd = {}, t change = {}".format(n, round(Cd_test, 2), round(t_change, 2)))
        best_val = list(best.x)
        data = np.column_stack((res.t.reshape(len(res.t),1), res.y[0,:].reshape(len(res.t),1), res.y[1,:].reshape(len(res.t),1), res.y[2,:].reshape(len(res.t),1),
        ...
        print("FAIL - Cd = {}, t change = {}".format(round(Cd_test, 2), round(t_change, 2)))
    n +=1'''
np.save(initial_path + "/Goddard_Problem/Testing/NN_Control/dataset_forNN_{}samplesTEST_.npy".format(n_samples), dataset)
#np.save("/home/francesco/Desktop/Git_workspace/IC4A2S/Goddard_Problem/Testing/NN_Control/cases_forNN_{}samples_noWeights.npy".format(n_samples), cases)
