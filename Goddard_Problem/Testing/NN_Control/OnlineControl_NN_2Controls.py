...
creator.create("Fitness", FitnessMulti, weights=(-1.0, -1.0))  # MINIMIZATION OF THE FITNESS FUNCTION
...
objects = []
with (open("hof_G2C_forNN_test.pkl", "rb")) as openfile:
    while True:
        try:
...
#dataset0 = np.load("dataset_forNN_100samples.npy", allow_pickle=True)
dataset = np.load("dataset_forNN_295samples_.npy", allow_pickle=True)
#cases = np.load("cases_forNN_200samples.npy", allow_pickle=True)
#dataset2 = np.load("dataset_forNN_50samples.npy", allow_pickle=True)
...
scale_units_x = [max(abs(dataset[:, 0])), max(abs(dataset[:, 1])),max(abs(dataset[:, 2])),max(abs(dataset[:, 3])),
                 max(abs(dataset[:, 4])), max(abs(dataset[:, 5])), max(abs(dataset[:, 6])), max(abs(dataset[:, 7]))] #, max(abs(dataset[:, 8]))]
scale_units_y = []
...
'''test_data_perc = 0.2
orig_len = len(datax)
train_data = tf.data.Dataset.from_tensor_slices((datax[:int(orig_len*(1-test_data_perc)),:], datay[:int(orig_len*(1-test_data_perc)),:]))
test_data = tf.data.Dataset.from_tensor_slices((datax[int(orig_len*(1-test_data_perc)):,:], datay[int(orig_len*(1-test_data_perc)):,:]))
BATCH_SIZE = 64
...
if save is True:
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(500, activation='relu', input_shape=(8,)),
        tf.keras.layers.Dense(500, activation='relu', kernel_initializer='RandomNormal'),
        #tf.keras.layers.Dropout(0.4),
        tf.keras.layers.Dense(500, activation='relu', kernel_initializer='RandomNormal'),
        #tf.keras.layers.Dropout(0.5),
        tf.keras.layers.Dense(500, activation='relu', kernel_initializer='RandomNormal'),
        #tf.keras.layers.Dropout(0.5),
        #tf.keras.layers.Dense(300, activation='relu', kernel_initializer='RandomNormal'),
        #tf.keras.layers.Dropout(0.4),
        tf.keras.layers.Dense(dataset.shape[1]-8)])
    model.compile(optimizer='adam', loss='mean_squared_error')
    model.summary()
    history = model.fit(datax, datay, epochs=10, validation_split=0.1, use_multiprocessing=True)
    ...
    plt.legend(loc='best')
    plt.show(block=True)
    model.save("mode_NN_2Controls_295samples.h5")
else:
    model = tf.keras.models.load_model('mode_NN_2Controls_295samples.h5')
...
while n < n_tot:
    ...
    res_stand = solve_ivp(partial(sys2GP, expr1=expr1, expr2=expr2, new_Cd=Cd_test, change_t=t_test, Control=False, model=[]), [0, tfin], x_ini, method='BDF')
    ...
    res2 = solve_ivp(partial(sys2GP, expr1=expr1, expr2=expr2, new_Cd=Cd_test, change_t=t_test, Control=True, model=model), [0.0, tfin], x_ini, method='BDF')
    r_cont = res2.y[0, :]
    theta_cont = res2.y[1, :]
    vr_cont = res2.y[2, :]
    vt_cont = res2.y[3, :]
    ...
    upper = 1.01
    fail = True
    if (Rref[-1]-obj.Re)*lower < (r_cont[-1]-obj.Re) < (Rref[-1]-obj.Re)*upper and Thetaref[-1]*lower < theta_cont[-1] < Thetaref[-1]*upper or \
            (Rref[-1]-obj.Re)*lower < (r_stand[-1]-obj.Re) < (Rref[-1]-obj.Re)*upper and Thetaref[-1]*lower < theta_stand[-1] < Thetaref[-1]*upper:
        if (Rref[-1]-obj.Re)*lower < (r_cont[-1]-obj.Re) < (Rref[-1]-obj.Re)*upper and Thetaref[-1]*lower < theta_cont[-1] < Thetaref[-1]*upper:  # tolerance of 1%
            print("NN success")
            nn_sc += 1
            fail = False
            data_nn.append([Cd_test, t_test])
        if (Rref[-1]-obj.Re)*lower < (r_stand[-1]-obj.Re) < (Rref[-1]-obj.Re)*upper and Thetaref[-1]*lower < theta_stand[-1] < Thetaref[-1]*upper:  # tolerance of 1%
            print("GP success")
            gp_sc += 1
            fail = False
            data_gp.append([Cd_test, t_test])
    else:
        diff_r = abs(Rref[-1]-r_cont[-1])/(Rref[-1]-obj.Re)
        diff_th = abs(Thetaref[-1] - theta_cont[-1])/Thetaref[-1]
        diff.append(np.max([diff_r, diff_th])*100)
        failure += 1
        data_fail.append([Cd_test, t_test])
        fail = True
    ...
    plt.plot(t_cont, (r_cont - obj.Re)/1e3, label="NN Cd: {}, t: {}".format(round(Cd_test, 2), round(t_test, 2)))
    
    plt.figure(3)
    #plt.plot(t_stand, vt_stand, label="Standard")
    plt.plot(t_cont, vt_cont, label="NN Cd: {}, t: {}".format(round(Cd_test, 2), round(t_test, 2)))
    
    plt.figure(4)
    #plt.plot(t_stand, m_stand, label="Standard")
    plt.plot(t_cont, m_cont, label="NN Cd: {}, t: {}".format(round(Cd_test, 2), round(t_test, 2)))
    
    plt.figure(5)
    #plt.plot(t_stand, vr_stand, label="Standard")
    plt.plot(t_cont, vr_cont, label="NN Cd: {}, t: {}".format(round(Cd_test, 2), round(t_test, 2)))

    plt.figure(6)
    #plt.plot(t_stand, np.rad2deg(theta_stand), label="Standard")
    plt.plot(t_cont, np.rad2deg(theta_cont), label="NN Cd: {}, t: {}".format(round(Cd_test, 2), round(t_test, 2)))
    n += 1
...
plt.figure(0)
plt.title("Number of successes {}/{}".format(n_tot-failure, n_tot))

for i in range(len(data_nn)):
    NN, = plt.plot(data_nn[i][1], data_nn[i][0], 'x', color='r')
for i in range(len(data_gp)):
    GP, = plt.plot(data_gp[i][1], data_gp[i][0], '.', color='b')

if data_gp and data_nn:
    plt.legend((NN, GP),('NN', 'GP'), loc='best')
elif not data_gp:
    plt.legend((NN,), ('NN',), loc='best')
elif not data_nn:
    plt.legend((GP,), ('GP',), loc='best')
#for i in range(len(data_fail)):
x = np.array(data_fail)[:,1]
y = np.array(data_fail)[:,0]
pt = plt.scatter(x, y, c=diff, vmax=5)
plt.colorbar(pt)
plt.show(block=True)
